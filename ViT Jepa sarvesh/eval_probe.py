...
What it does:
    1. Loads a saved ViT-JEPA checkpoint (model weights, predictor discarded at eval).
    2. Freezes the online encoder.
    3. Iterates train/val/test splits, encoding each 16-frame window with
       model.encode() → mean-pooled (B, 256) feature vector.
    4. Aggregates per trajectory (mean-pool windows from same simulation).
    5. Trains TWO heads on frozen features:
        a) Linear probe: nn.Linear(256, 2) regressing to (alpha, zeta).
...
Usage:
    python eval_probe.py \
        --checkpoint /scratch/sb10583/checkpoints/vit-jepa-sarvesh/best.pt \
        --data-dir   /scratch/sb10583/data/data \
        --output-json vit-jepa-sarvesh_results.json
"""
...
def build_encoder_from_checkpoint(checkpoint_path: Path,
    ...
    cfg  = ckpt.get("config", {})
    m    = cfg.get("model", cfg)  # flat cfg or nested under "model"

    model = ViTJEPA(
        in_channels = m.get("in_channels",  11),
        embed_dim   = m.get("embed_dim",   256),
        depth       = m.get("depth",         6),
        num_heads   = m.get("num_heads",      8),
        mlp_ratio   = m.get("mlp_ratio",    4.0),
        img_size    = m.get("crop_size",    224),
        patch_size  = m.get("patch_size",    16),
        tubelet     = m.get("tubelet",        2),
        num_frames  = m.get("num_frames",    16),
        pred_dim    = m.get("pred_dim",     128),
        pred_depth  = m.get("pred_depth",     4),
        pred_heads  = m.get("pred_heads",     4),
    )
    state = ckpt["model"]
...
def train_linear_probe(X_train: np.ndarray, y_train: np.ndarray,
    ...
    # Z-score features with train statistics
...
def main():
    ...
    parser.add_argument("--output-json",   default="vit-jepa-sarvesh_results.json", type=Path)
