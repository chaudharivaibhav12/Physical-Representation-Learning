...
We present a systematic study of self-supervised representation learning for physical simulations, evaluated on the `active_matter` dataset — a benchmark of 2D active nematic fluid dynamics governed by two continuous physical parameters, α (active dipole strength) and ζ (steric alignment). We train eleven model variants from scratch across five method families: convolutional JEPA with VICReg and EMA objectives, VICReg with a 3D Vision Transformer encoder, ViT-JEPA with temporal predictive coding and token-level VICReg, ViT-JEPA with EMA target encoders and spatiotemporal block masking, and VideoMAE masked autoencoding. Representations are evaluated by predicting α and ζ from a frozen encoder via linear probing and kNN regression (MSE on z-score normalized targets; random baseline ≈ 1.0). Our best model, a Conv-JEPA with EMA target encoder and no augmentation, achieves a linear probe MSE of **0.011** on α and **0.148** on ζ on the validation set. The 3D ViT with VICReg (100 epochs, full dataset) achieves **0.062** / **0.195** (LP α/ζ val) with healthy representations (embedding std = 0.37). Key findings: EMA-based collapse prevention consistently outperforms VICReg for convolutional encoders; training scale is critical for ViT models; and α is universally easier to predict than ζ across all architectures.
