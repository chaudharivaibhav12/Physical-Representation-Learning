"""
Linear probe + kNN evaluation for VICReg ViT encoder (vision-transformer-v2).

What it does:
    1. Loads a saved VICReg checkpoint (encoder weights; projector discarded).
    2. Freezes the encoder.
    3. Iterates train/val/test splits, encoding each 16-frame window with
       encoder.forward_pooled() → (B, 384) feature vector.
    4. Aggregates per trajectory (mean-pool windows from same simulation).
    5. Trains TWO heads on frozen features:
        a) Linear probe: nn.Linear(384, 2) regressing to (alpha, zeta).
...
Usage:
    python eval_probe.py \
        --checkpoint /scratch/sb10583/checkpoints/vicreg-v4/best.pt \
        --data-dir   /scratch/sb10583/data/data \
        --output-json vicreg-v2_results.json
"""
...
@torch.no_grad()
def extract_features(encoder: nn.Module, data_dir: str, split: str,
...
def main():
    ...
    parser.add_argument("--output-json",   default="vicreg-v2_results.json", type=Path)
    ...
    Xtr, ytr, ids_tr = extract_features(encoder, args.data_dir, "train", device,
                                         args.batch_size, args.num_workers,
                                         args.stride, not args.no_amp)
    Xva, yva, ids_va = extract_features(encoder, args.data_dir, "valid", device,
                                         args.batch_size, args.num_workers,
                                         args.stride, not args.no_amp)
    Xte, yte, ids_te = extract_features(encoder, args.data_dir, "test",  device,
                                         args.batch_size, args.num_workers,
                                         args.stride, not args.no_amp)
