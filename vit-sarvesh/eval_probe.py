"""
Linear probe + kNN evaluation for ViT-JEPA encoder.

Steps:
  1. Load encoder from checkpoint (predictor discarded).
  ...
  4. Linear probe: nn.Linear(256, 2) trained with AdamW + MSE on z-scored labels.
...
Usage:
  python eval_probe.py --checkpoint /scratch/sb10583/checkpoints/vit-jepa-sarvesh/best.pt
...
def load_encoder(checkpoint_path: Path, device: torch.device) -> nn.Module:
    ...
    model = ViTJEPA(
        ...
        pred_heads  = cfg.get("pred_heads",      4),
    )
    state = ckpt["model"]
    ...
    n = sum(p.numel() for p in model.online_encoder.parameters())
    print(f"[load ] online encoder {n:,} params, frozen, on {device}")
    return model
...
@torch.no_grad()
def extract_features(model, data_dir: str, split: str,
                     device: torch.device, batch_size: int,
                     num_workers: int, stride: int = 16):
    ds = WithSimID(ActiveMatterDataset(
...
def norm_labels(y, mean, std):   return (y - mean) / std
def denorm_labels(z, mean, std): return z * std + mean
...
def fit_knn(X_tr, y_tr, X_va, y_va, lbl_mean, lbl_std, ks=(1, 3, 5, 10, 20)):
...
def main():
    ...
    X_tr, y_tr, ids_tr = extract_features(model, args.data_dir, "train", device, args.batch_size, args.num_workers, args.stride)
    X_va, y_va, ids_va = extract_features(model, args.data_dir, "valid", device, args.batch_size, args.num_workers, args.stride)
    X_te, y_te, ids_te = extract_features(model, args.data_dir, "test",  device, args.batch_size, args.num_workers, args.stride)
